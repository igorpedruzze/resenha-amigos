...
export default function SuperAdminDashboard() {
  ...
  const [pendingOrgChanges, setPendingOrgChanges] = useState<Record<number, number>>({});
  const [isSavingOrg, setIsSavingOrg] = useState<number | null>(null);
  ...
  const updateOrgPlan = async (orgId: number, planId: number) => {
    ...
    try {
      ...
      if (res.ok) {
        setOrgs(orgs.map(org => org.id === orgId ? { ...org, plan_id: planId } : org));
      ...
      <aside className="w-72 bg-slate-900 flex flex-col fixed inset-y-0 z-50">
        <div className="p-8">
          <div className="flex items-center gap-3 text-emerald-400 mb-8">
            ...
            <span className="text-xl font-black tracking-tighter text-white uppercase">Super Admin</span>
          </div>
          
          <nav className="space-y-2">
            <button 
              onClick={() => setActiveTab('orgs')}
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${activeTab === 'orgs' ? 'bg-white/10 text-white font-medium' : 'text-white/70 hover:bg-white/5 hover:text-white'}`}
            >
            ...
            <button 
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${activeTab === 'plans' ? 'bg-white/10 text-white font-medium' : 'text-white/70 hover:bg-white/5 hover:text-white'}`}
            >
            ...
            <button 
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${activeTab === 'settings' ? 'bg-white/10 text-white font-medium' : 'text-white/70 hover:bg-white/5 hover:text-white'}`}
            >
            ...
            <button 
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${activeTab === 'appearance' ? 'bg-white/10 text-white font-medium' : 'text-white/70 hover:bg-white/5 hover:text-white'}`}
            >
              <Palette className="size-5" />
              <span>Aparência Home</span>
            </button>
          </nav>
        </div>
        ...
        <div className="mt-auto p-8">
          <button 
            ...
            className="w-full py-3 bg-white/10 hover:bg-white/20 text-white text-sm font-medium rounded-xl transition-colors flex items-center justify-center gap-2"
          >
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-slate-900 flex items-center gap-2">
              {activeTab === 'orgs' && 'Organizações'}
              {activeTab === 'plans' && 'Planos de Assinatura'}
              {activeTab === 'settings' && 'Configurações'}
              {activeTab === 'appearance' && 'Aparência Home'}
            </h1>
            <p className="text-slate-500">Gerenciamento global da plataforma</p>
          </div>
        </header>

        {/* Global Stats - Only show on Orgs tab or a dedicated Dashboard tab if we had one */}
        {activeTab === 'orgs' && (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          <select 
                            value={pendingOrgChanges[org.id] !== undefined ? pendingOrgChanges[org.id] : (org.plan_id || '')} 
                            onChange={(e) => setPendingOrgChanges({ ...pendingOrgChanges, [org.id]: parseInt(e.target.value) })}
                          ...
                          {pendingOrgChanges[org.id] !== undefined && (
                            <button 
                              onClick={() => updateOrgPlan(org.id, pendingOrgChanges[org.id])}
                              disabled={isSavingOrg === org.id}
                              className="bg-emerald-600 text-white px-2 py-1 rounded-lg text-[10px] font-bold hover:bg-emerald-700 transition-colors disabled:opacity-50"
                            >
                              {isSavingOrg === org.id ? '...' : 'SALVAR'}
                            </button>
                          )}
                        </div>
                      </td>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <button 
                    onClick={async () => {
                      setIsSavingSettings(true);
                      await Promise.all([
                        updateSetting('platform_name', platformSettings.platform_name),
                        updateSetting('hero_title', platformSettings.hero_title),
                        updateSetting('hero_subtitle', platformSettings.hero_subtitle),
                        updateSetting('footer_text', platformSettings.footer_text)
                      ]);
                      setIsSavingSettings(false);
                      alert('Configurações salvas com sucesso!');
                    }}
                    disabled={isSavingSettings}
                    className="bg-emerald-600 text-white px-4 py-2 rounded-xl hover:bg-emerald-700 transition-colors text-sm font-medium"
                  >
                    {isSavingSettings ? 'Salvando...' : 'Salvar Textos'}
                  </button>
                </div>
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 space-y-6">
                <div className="flex justify-between items-center">
                  ...
                  <button 
                    onClick={() => updateSetting('primary_color', platformSettings.primary_color)}
                    disabled={isSavingSettings}
                    className="bg-emerald-600 text-white px-4 py-2 rounded-xl hover:bg-emerald-700 transition-colors text-sm font-medium"
                  >
                    Salvar Cor
                        ...
                        {platformSettings.platform_logo && (
                          <div className="relative group">
                            ...
                            <div className="absolute inset-0 bg-emerald-500/10 opacity-0 group-hover:opacity-100 transition-opacity rounded-lg flex items-center justify-center">
                              <span className="text-[10px] font-bold text-emerald-600 bg-white px-2 py-0.5 rounded shadow-sm">Atual</span>
                            </div>
                          </div>
                        )}
                        <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-slate-200 rounded-2xl cursor-pointer hover:bg-slate-50 transition-colors relative overflow-hidden">
                          {isUploading ? (
                            <div className="flex flex-col items-center">
                              <div className="size-6 border-2 border-emerald-500 border-t-transparent rounded-full animate-spin mb-2"></div>
                              <span className="text-[10px] text-slate-500">Subindo...</span>
                            </div>
                          ) : (
                            <>
                          ...
                          <input type="file" className="hidden" disabled={isUploading} onChange={(e) => e.target.files?.[0] && handleFileUpload(e.target.files[0], 'logo')} />
                        </label>
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="space-y-4">
                        ...
                        <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-slate-200 rounded-2xl cursor-pointer hover:bg-slate-50 transition-colors relative overflow-hidden">
                          ...
                          <input type="file" className="hidden" disabled={isUploading} onChange={(e) => e.target.files?.[0] && handleFileUpload(e.target.files[0], 'favicon')} />
                        </label>
                      </div>
                    </div>
                  </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 space-y-6">
              <div className="flex justify-between items-center">
                ...
                <label className="bg-emerald-600 text-white px-4 py-2 rounded-xl hover:bg-emerald-700 transition-colors font-medium flex items-center gap-2 cursor-pointer">
                  ...
                  {isUploading ? 'Subindo...' : 'Adicionar Banner'}
                  <input type="file" className="hidden" disabled={isUploading} onChange={(e) => e.target.files?.[0] && handleFileUpload(e.target.files[0], 'banner')} />
                </label>
              </div>
